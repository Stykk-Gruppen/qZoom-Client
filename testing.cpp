...
static int output_audio_frame(AVFrame *frame)
{
    ...
    printf("audio_frame n:%d nb_samples:%d pts:\n",
                              ...
                              AVCodecContext **dec_ctx, AVFormatContext *fmt_ctx, enum AVMediaType type)
{
    ...
    if (ret < 0) {
        fprintf(stderr, "Could not find %s stream in input file '%s'\n",
                av_get_media_type_string(type), "v4l2");
        return ret;
    } else {
        stream_index = ret;
...
void testing::run()
{
    ...
    video_dst_filename = "julyTestingVideo.ismv";
    audio_dst_filename = "julyTestingAudio.isma";

    AVInputFormat* videoInputFormat = av_find_input_format("v4l2");
    if(videoInputFormat == NULL)
    {
        fprintf(stderr, "Could not open v4l2");
        exit(1);
    }
    ...
    if (video_stream)
        printf("Demuxing video from file '%s' into '%s'\n", "v4l2", video_dst_filename);
    if (audio_stream)
        printf("Demuxing audio from file '%s' into '%s'\n", "v4l2", audio_dst_filename);
